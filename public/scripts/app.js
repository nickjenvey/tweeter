...
$(document).ready(function() {
  ...
  const loadTweets = () => {
    $.getJSON("/tweets", (tweet) => {
      renderTweets(tweet);
      $(".tweet__media-controls a").click((e) => {        // Like button action
        e.preventDefault();
        const tweetId = e.target.closest(".tweet").dataset.id;
        $.post(`/tweets/${tweetId}/likes`, (likes) => {
          $(e.target).parent().find(".likes").text(likes.likes);
        });
      });
    });
  };
  ...
  const createTweetElement = (tweet) => {
    const {_id, user, content, created_at} = tweet;
    const header = $("<header/>")
